...
class ProtvistaStructure extends HTMLElement implements NightingaleElement {
  ...
  get colorThemes(): string {
    return this.getAttribute("colorthemes");
  }

  set colorThemes(colorThemes: string) {
    this._colorThemes = colorThemes
    this.setAttribute("colorThemes", colorThemes);
  }
  ...
  async connectedCallback(): Promise<void> {
    ...
    this.updateUrls();
    this._colorThemes = this.getAttribute('colorthemes')
    ...
    this._structureViewer.loadStructureFromData(this._file, 'cif').then(() => {
      this._structureViewer.setZoomSpeed()
      this._structureViewer.setViewerDisplay(this._colorThemes)
    })
  }
  ...
  static get observedAttributes(): string[] {
    return ["highlight", "structureid", "accession", "height", 'colorthemes', 'file'];
  }
    ...
    if (oldVal !== newVal) {
      ...
      switch (attrName) {
        ...
        case "height":
          ...
          break;
        case "colorthemes":
          this._colorThemes = newVal
          this._structureViewer && this._structureViewer.setViewerDisplay(this._colorThemes)
          break
        case 'file':
          this._file = newVal
          this._structureViewer && this._structureViewer.loadStructureFromData(this._file, 'cif').then(() => {
            this._structureViewer.setZoomSpeed()
            this._structureViewer.setViewerDisplay(this._colorThemes)
          })
          break;
